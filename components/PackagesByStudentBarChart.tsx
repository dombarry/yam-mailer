...
import type { DashboardPackage } from "@/lib/types"
    ...
    <>
      <div className="w-full h-[50vh] flex flex-row justify-start items-start mt-4">
        <ResponsiveContainer width="70%" height="100%">
          ...
          >
            ...
            <Legend
              ...
              content={<p>Distribution of Packages by Student</p>}
            />
        ...
        <div className="flex flex-col items-start my-11 border-gray-900 px-2 w-[40%]">
          ...
          <TypographyP>
            - The top decile of students contribute a total of{" "}
            <strong>{numPackagesInPercentile(data.slice(), 10)}</strong> packages, which is{" "}
            <strong>{((100 * numPackagesInPercentile(data.slice(), 10)) / totalPackages).toFixed(1)}%</strong> of the
            total.
          </TypographyP>
          <TypographyP>
            - The top quartile of students contribute a total of{" "}
            <strong>{numPackagesInPercentile(data.slice(), 25)}</strong> packages, which is{" "}
            <strong>{((100 * numPackagesInPercentile(data.slice(), 25)) / totalPackages).toFixed(1)}%</strong> of the
            total.
          </TypographyP>
          <TypographyP>
            - The top student had <strong>{data[data.length - 1].num_packages}</strong> packages, while the median
            student had <strong>{getMedianNumPkgs(data)}</strong> packages. On average, a student received{" "}
            <strong>{avgPackages(data)}</strong> packages.
          </TypographyP>
        </div>
      </div>
    </>
