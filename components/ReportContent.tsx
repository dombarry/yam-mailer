...
import type { DashboardLogged, Hall, HallLogged, HallStats } from "@/lib/types"
...
export default function ReportContent() {
  ...
  const loggedAndRestricted = restrictToInterval(loggedData?.flatMap((d) => d.packages) ?? [], INTERVAL)
  ...
  const downloadData = () => {
    const tempcsv = loggedData!
      .flatMap((d) => d.packages)
      .filter((d) => new Date(d.ingestedTime) >= startDate && new Date(d.ingestedTime) <= endDate)
      .map((d) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header activeTab="diagnostics" userName={user?.name} />

      <div className="flex flex-col justify-start items-center w-full h-full min-h-[100vh] p-4">
        <TypographyH1 className="mt-5 mb-1 font-sans text-3xl">
          {HALL} Hall Report -- {INTERVAL}
        </TypographyH1>
              ...
              <TabsContent value="stats">
                <Card className={cardCn}>
                  <CardHeader className="mb-0 pb-2">
                    ...
                    <CardDescription>Figures generated from package data in {INTERVAL}.</CardDescription>
                  </CardHeader>
                  <CardContent className="my-3">
                    <div>
                      <div className="flex flex-row items-center justify-start">
                        <TypographyP className="mr-2">
                          A total of <strong>{loggedAndRestricted.length.toString()}</strong> packages were delivered to{" "}
                          <strong>{getNumStudents(loggedAndRestricted).toString()}</strong> students. On average,{" "}
                          <strong>{getNumDailyAvgPkgs(loggedAndRestricted).toFixed(0)}</strong> packages were picked up
                          per day. The median time to pickup for a package was{" "}
                          <strong>{getMedianTimeToPickup(loggedAndRestricted).toFixed(1)}</strong> hours.
                        </TypographyP>
                      </div>
                    </div>
                    <Total data={restrictCombinedData(combineData(data, loggedData))} halls={[lower(HALL)]} />
                    <PackagesByStudentBarChart data={loggedAndRestricted} />
                  </CardContent>
                </Card>
              </TabsContent>
              <TabsContent value="data">
                <Card className={cardCn}>
                  <CardHeader className="flex flex-row justify-between w-full items-center py-0 my-2">
                    ...
                    <Button variant="outline" className="" onClick={downloadData}>
                      <Download className="h-4 w-4" />
                    </Button>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    <DataTable data={loggedAndRestricted} columns={columns} />
                  </CardContent>
                </Card>
              </TabsContent>
            </Tabs>
