...
const getLoggedPackages = async (hall: string): Promise<DashboardLogged[]> => {
  ...
  const dashboardPackages: DashboardLogged[] = packages
    .map((p) => {
      return {
        packageId: p.packageId,
        name: `${p.First} ${p.Last}`,
...
const dataByMonths = (
  ...
  const sortFunc = (data: MonthData[]): MonthData[] => {
    ...
    data.sort((a, b) => {
      ...
      const aYear = Number.parseInt(a.name.split(" ")[1])
      const bMonth = months.indexOf(b.name.split(" ")[0] as Month)
      const bYear = Number.parseInt(b.name.split(" ")[1])
      if (aYear === bYear) {
...
const dataByWeeks = (
  ...
  const sortFunc = (data: WeekData[]): WeekData[] => {
    ...
    data.sort((a, b) => {
      const aYear = Number.parseInt(a.name.split("/")[2])
      const bYear = Number.parseInt(b.name.split("/")[2])
      if (aYear !== bYear) {
        return aYear - bYear
      }
      const aMonth = Number.parseInt(a.name.split("/")[0])
      const bMonth = Number.parseInt(b.name.split("/")[0])
      if (aMonth !== bMonth) {
        return aMonth - bMonth
      }
      const aDay = Number.parseInt(a.name.split("/")[1])
      const bDay = Number.parseInt(b.name.split("/")[1])
      return aDay - bDay
    })
    return data
  }
...
const dataByGranularity = (
  ...
  handlerFunc: (
    ...
    datum: MonthData | WeekData | DayData,
    name: Month | string,
    hall: Hall,
  ) => void,
  inbound: boolean,
...
const avgTimeToPickup = (data: HallLogged[], granularity: Granularity): MonthData[] | WeekData[] | DayData[] => {
  ...
  if (granularity !== "month") {
    finalData.sort((a, b) => {
      const y1 = Number.parseInt(a.name.split("/")[2])
      const y2 = Number.parseInt(b.name.split("/")[2])
      ...
      if (y1 !== y2) {
        ...
        return Number.parseInt(d1) - Number.parseInt(d2)
      } else {
        return Number.parseInt(m1) - Number.parseInt(m2)
      }
    })
  } else {
    finalData.sort((a, b) => {
